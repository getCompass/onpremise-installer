...
parser = scriptutils.create_parser(
    description="Скрипт для бекапа всего приложения.",
    usage="python3 script/backup_all_data.py [-v VALUES] [-e ENVIRONMENT] [-dst DESTINATION]  [--yes] [--backups-folder BACKUPS_FOLDER] [--backup-name-format BACKUP_NAME_FORMAT] [--free-threshold-percent FREE_THRESHOLD_PERCENT] [--auto-cleaning-limit AUTO_CLEANING_LIMIT] [--userbot-notice-chat-id USERBOT_NOTICE_CHAT_ID] [--userbot-notice-token USERBOT_NOTICE_TOKEN] [--userbot-notice-domain USERBOT_NOTICE_DOMAIN] [--userbot-notice-text USERBOT_NOTICE_TEXT]",
    epilog="Пример: python3 script/backup_all_data.py -v compass -e production -dst /home/compass --yes --backups-folder backups --backup-name-format %d_%m_%Y --free-threshold-percent 10 --auto-cleaning-limit 15 --userbot-notice-chat-id PEHneUlL7nM... --userbot-notice-token f47f9384-sk4f-4d1c-8193-7a9b9384952e --userbot-notice-domain https://example.com/ --userbot-notice-text \"Ошибка при создании бэкапа на сервере!\"",
)
...
parser.add_argument("-dst", "--destination", required=True, type=str,
                    help="Место, куда будет отправлена резервная копия. Можно указать другой сервер, чтобы отправить через rsync", )
