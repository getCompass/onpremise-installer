...
parser.add_argument("--is_logs", required=False, default=1, type=int, help="нужны ли логи старта репликации")
...
def start():
    ...
    if mysql_type == "team":
        ...
        logging.info("Start replication for companies")

        # проходимся по каждому пространству
        for space_id, space_config_obj in space_config_obj_dict.items():

            logging.info("Start replication for company %s" % space_id)
        ...
        logging.info("Start replication for monolith")
...
def mysql_start_replication(found_container: docker.models.containers.Container, change_master_mysql_command: str, mysql_host: str, mysql_user: str, mysql_pass: str, space_id: int):
    ...
    if is_logs:
        print("Ожидаем завершение репликации mysql")
    subprocess.run(
        [
            "python3",
