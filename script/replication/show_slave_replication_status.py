...
def start():
    ...
    if current_values.get("service_label") is None or current_values.get("service_label") == "":
        ...
        if confirm != "y":
            scriptutils.error("Заверешаем выполнение скрипта")
    ...
    if mysql_type == "team":
        ...
        for space_id, space_config_obj in space_config_obj_dict.items():
            print("Получаем статус репликации в компании %s" % space_id)
            found_container = scriptutils.find_container_mysql_container(client, mysql_type, domino_id, space_config_obj.port)
            mysql_show_slave_replication_status(found_container, mysql_host, mysql_user, mysql_pass)
...
def mysql_show_slave_replication_status(found_container: docker.models.containers.Container, mysql_host: str, mysql_user: str, mysql_pass: str):
    ...
    slave_status_text = slave_status_text + "\n" + "Slave_IO_Running: %s" % result.get("Slave_IO_Running")
    slave_status_text = slave_status_text + "\n" + "Slave_SQL_Running: %s" % result.get("Slave_SQL_Running")
    slave_status_text = slave_status_text + "\n" + "Seconds_Behind_Master: %s" % result.get("Seconds_Behind_Master")

    slave_status_text = slave_status_text + "\n\n" + "Last_IO_Error: %s" % result.get("Last_IO_Error")
    slave_status_text = slave_status_text + "\n" + "Last_SQL_Error: %s" % result.get("Last_SQL_Error")
    print(slave_status_text)
