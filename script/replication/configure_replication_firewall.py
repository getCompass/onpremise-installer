...
def check_rules(tool: str, rules: List[str]) -> bool:
    ...
    # iptables -S печатает строковые значения (комментарий) в кавычках - нормализуем
    return [rule.replace('"', '') for rule in actual] == rules
...
def ensure_firewall(current_values: dict) -> bool:
    ...
    peer_ips = [str(current_values["peer_host"]).strip()]
...
def main():
    ...
    if args.check:
        ...
        rules = build_rules([str(current_values["peer_host"]).strip()], ingress_subnet,
                            list(MANTICORE_PORTS), collect_mysql_ports(current_values))
        tool = detect_tool()
        if tool is None:
